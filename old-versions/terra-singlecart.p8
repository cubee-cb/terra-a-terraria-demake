...
-- player alive
 ...
 repeat
  ...
  if pyv>0 and (py>1020 or fget(l,0) or fget(r,0) or (l==14 or r==14) and not btn(3)) then
 ...
 tile,helditem,canturn,digging=mget(actx,acty),inventory[invsel],true,false
 ...
 if btn(5) then
  ...
  if hitype=="axe" or hitype=="pick" then
   ...
   if til~=0 and((hitype=="axe" and tile==83) or (hitype=="pick" and fget(tile,1) and mget(actx,acty-1)~=83))then
    ...
    pdigt+=1
   	 ...
   	 do
   	  b=mget(actx,acty+ty)
  	   additem(drops[b] or b,px+digx,py+digy+ty*8,1,mget(actx,acty+ty)==9 and 20 or "block")
   	  mset(actx,acty+ty,0)
  	   ty-=1
  	  end
  ...
  -- place block
  elseif hitype=="block" and pswing==hiuse then
...
for i in all(mobs) do
 ...
 elseif ai=="slime" then
  ...
  v=
  true,i.jumpt or 60,i.d or sgn(rnd(2)-1),false,
...
function generate()
 ...
 genprint"sculpting the land"
   ...
   -- dirt
    ...
    cs,js=128-72*worlddir,128+72*worlddir
